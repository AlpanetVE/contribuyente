...
$('#usr-update-form').formValidation({
	...
	$('.modal-update-user').on('show.bs.modal', function (e) {
		...
		if(usuarios_id>0){
			$.ajax({
				url: "function/f_usuarios.php", // la URL para la petici&oacute;n
	            ...
	            success: function (data) {
	            	...
	            	if (data.result === 'OK') {
				            	$('.modal-update-user #update_seudonimo').val(data.campos.u_seudonimo);
				            $('.modal-update-user #update_nombre').val(data.campos.u_nombre);
										$('.modal-update-user #update_apellido').val(data.campos.u_apellido);
										$('.modal-update-user #update_cedula').val(data.campos.u_cedula);
										$('.modal-update-user #update_cargo').val(data.campos.u_cargo);
